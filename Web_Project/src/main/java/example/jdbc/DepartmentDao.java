...
//		try-with-reource autoclosable open under this->()
		try (Connection dbconnection = JDBCUtils.buildConnection();
				...
				ResultSet rs = smt.executeQuery("select * from department")) {
//			all record for loop
			while (rs.next()) {
...
//			get data 3 values of ?
	...
	@Override
	public void update(Department modifydept) {
		// single suery modify namr, loc, and name also loc we have 3 possiblities
//											index 	1		2				 3
		String sqlQuery = "update department set dname = ?, loc = ? where deptno = ?";
		try (Connection dbconnection = JDBCUtils.buildConnection();
				PreparedStatement pstmt = dbconnection.prepareStatement(sqlQuery)) {

//			get data 3 values of ?
			int deptNo = modifydept.getDeptNo();
			String dName = modifydept.getdName();
			String loc = modifydept.getLoc();

			pstmt.setInt(3, deptNo);
			pstmt.setString(1, dName);
			pstmt.setString(2, loc);
			int updateCount = pstmt.executeUpdate();
			System.out.println("The UpdateCount: " + updateCount + " record Updated");
	...
	public void delete(Integer deptno) {
		String sqlQuery = "delete from department where deptno = ?";
		try (Connection dbconnection = JDBCUtils.buildConnection();
				PreparedStatement pstmt = dbconnection.prepareStatement(sqlQuery)) {

			pstmt.setInt(1, deptno);
			int deletCount = pstmt.executeUpdate();
			if (deletCount == 0)
				System.err.println(" Department not exist with given ID: " + deptno);
			else
				System.out.println("deleted records : " + deletCount);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
